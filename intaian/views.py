...
def get_rwgolongan(request, nip):
    payload = ''
    headers = {
    ...
    for hasil in sumber:
        ...
        try :
            ...
            print (e)
        return render(request, 'intaian/index.html', {'x': xyz, 'sumber': sumber})
    return render(request, "intaian/index.html")


def get_pasangan(request, nip):
    payload = ''
    headers = {
        'accept': 'application/json',
        'Auth': 'bearer ' + ViewAuthorization(request),
...
def get_pendidikan(request, nip):
    payload = ''
    headers = {
        'accept': 'application/json',
        'Auth': 'bearer ' + ViewAuthorization(request),
    ...
    datautama.request("GET", "/apisiasn/1.0/pns/rw-pendidikan/" + nip, payload, headers)
    res = datautama.getresponse()
    data = res.read()
    xyz = json.loads(data.decode("utf-8"))
    sumber = xyz.get('data')
    rwpendidikan = ModelRwPendidikan.objects.all()
    for hasil in sumber:
        try :
            rwpendidikan.update_or_create (
                idsiasn=hasil['id'] ,
                idpns_id=hasil['idPns'],
                nipbaru=hasil['nipBaru'],
                niplama=hasil['nipLama'],
                pendidikanid=hasil['pendidikanId'],
                pendidikannama=hasil['pendidikanNama'],
                tkpendidikanid=hasil['tkPendidikanId'],
                tkpendidikannama=hasil['tkPendidikanNama'],
                tahunlulus=hasil['tahunLulus'],
                tgllulus=hasil['tglLulus'],
                ispendidikanpertama=hasil['isPendidikanPertama'],
                nomorijasah=hasil['nomorIjasah'],
...
def get_data (request, nip):
    ...
    xyz = json.loads(data.decode("utf-8"))
    sumber = xyz.get('data')
    print(sumber.get('nipBaru'))
    if not Datautama.objects.filter(idpns=sumber.get('id')).exists():
        try :
            Datautama.objects.update_or_create(
                idpns =sumber.get( 'id' ),
            ...
            print (e)
    return render(request, 'intaian/index.html', {'sumber':sumber})
